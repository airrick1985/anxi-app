...
/**
 * ✅ 【修改版】設定使用者的自訂權限 (Custom Claim)
 * 改為使用 Email 來查找使用者
 * @param {object} data - 包含 { email, role } 的物件
 */
exports.setUserRole = onCall(async (request) => {
    ...
    // 步驟 1: 驗證呼叫者的權限
    //if (request.auth.token.role !== '系統管理員') {
     //   throw new HttpsError('permission-denied', '您的權限不足，無法執行此操作。');
    //}

    //if (!email || !role) {
      //  throw new HttpsError('invalid-argument', '缺少 email 或 role 參數。');
   // }

    // 步驟 2: 根據 Email 找到對應的 Firebase Auth 使用者
    try {
        const user = await admin.auth().getUserByEmail(email);
        
        // 步驟 3: 為該使用者設定 Custom Claim
        await admin.auth().setCustomUserClaims(user.uid, { role: role });

        // 步驟 4 (可選但建議): 同步更新 Firestore user 文件中的 role 欄位
        // 由於文件 ID 不是 UID 或 Email，我們需要查詢來找到對應文件
        ...
        return { success: true, message: `成功將使用者 ${email} 的角色設為 ${role}` };

    } catch (error) {
        console.error("設定使用者角色時發生錯誤:", error);
        if (error.code === 'auth/user-not-found') {
            throw new HttpsError('not-found', `找不到 Email 為 ${email} 的使用者。`);
        }
