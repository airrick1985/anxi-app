function handleLogin(payload) {
  ...
  const permissionsIndex = header.indexOf('功能權限'); // Get permissions column index
    ...
    if (row[phoneIndex] === payload.key) {
      ...
      if (permissionsIndex !== -1) {
        ...
        if (rawPermissions && typeof rawPermissions === 'string' && rawPermissions.trim() !== '') {
          ...
          Logger.log("[handleLogin] User " + userName + " permissions cell is empty or not a string. Defaulting to [].");
        }
      } else {
