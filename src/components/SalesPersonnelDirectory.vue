<template>
  <v-card class="personnel-directory" elevation="2">
    ...
    <div v-else class="directory-scroll">
      <div class="personnel-row directory-heading" aria-hidden="true">
        <span v-if="sorting">排序</span><span>姓名／電話</span><span>職位</span><span>Email</span><span>操作</span>
      </div>
...
.personnel-row { display: grid; grid-template-columns: minmax(180px, 1fr) minmax(140px, .8fr) minmax(180px, 1fr) 80px; align-items: center; gap: 16px; padding: 10px 20px; border-bottom: 1px solid rgba(var(--v-border-color), .08); }
...
.directory-heading:has(span:nth-child(5)), .is-sorting .personnel-row { grid-template-columns: 120px minmax(160px, 1fr) minmax(120px, .8fr) minmax(120px, 1fr) 80px; }
...
@media (max-width: 959px) {
  ...
  .personnel-row-actions { grid-column: 2; grid-row: 1 / span 3; }
  .personnel-positions, .personnel-email { grid-column: 1; }
  .personnel-order { grid-column: 1 / -1; }
  .is-sorting .personnel-identity { grid-row: 2; }
  .is-sorting .personnel-row-actions { grid-row: 2 / span 3; }
