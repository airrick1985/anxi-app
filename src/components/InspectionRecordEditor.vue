    ...
    <v-card>
      ...
      <v-card-text style="max-height: 85vh;">
        <v-form ref="form" v-model="valid">
          <v-container>
            <v-row dense>
              <v-col cols="12" sm="6">
                <v-text-field label="建案" :model-value="projectName" readonly variant="outlined"></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field label="戶別" :model-value="unitId" readonly variant="outlined"></v-text-field>
              </v-col>

              <v-col cols="12" sm="6">
                <VueDatePicker v-model="formData.inspectionDate" locale="zh-tw" auto-apply enable-time-picker:false
                  format="yyyy/MM/dd" placeholder="選擇驗屋日期" :input-class-name="'dp-custom-input'" :clearable="false"
                  required />
              ...
              <v-col cols="12">
                ...
                <v-chip-group v-model="formData.phase" mandatory column color="primary">
                  <v-chip v-for="option in options.phase" :key="option.id" :value="option.value" filter
                    variant="outlined">
                    {{ option.value }}
                  </v-chip>
                </v-chip-group>
              ...
              <v-col cols="12" class="mb-2">
                ...
                <div class="d-flex flex-wrap ga-2 align-center">
                  <div v-for="(photo, index) in formData.photos" :key="index" class="photo-preview-container">
                    <v-img :src="photo.previewUrl || photo.url" aspect-ratio="1" cover width="80"
                      class="rounded border">
                      <v-btn icon="mdi-close-circle" color="red" size="x-small" variant="text" class="photo-delete-btn"
                        @click="removePhoto(index)"></v-btn>
                    </v-img>
                  </div>
                  <v-btn v-if="formData.photos.length < maxPhotos" icon="mdi-camera-plus" variant="outlined"
                    color="grey-darken-1" @click="triggerPhotoInput"></v-btn>
                </div>
                <input type="file" ref="photoInput" accept="image/*" multiple @change="handlePhotoSelected"
                  style="display: none;" />
                <PhotoEditor v-model="showPhotoEditor" :file="photoToEdit" @done="handlePhotoEdited"
                  @cancel="cancelPhotoEdit" />
              </v-col>

              <v-col cols="12" sm="6" md="4">
                <v-select v-model="formData.area" :items="options.area" item-title="value" item-value="value"
                  label="檢查區域" variant="outlined" class="large-text-select" :menu-props="{ class: 'large-select-menu' }"
                  required :rules="[v => !!v || '必填']"></v-select>
              </v-col>
              <v-col cols="12" sm="6" md="4">
                <v-select v-model="formData.category" :items="options.category" item-title="value" item-value="value"
                  label="工程種類" variant="outlined" class="large-text-select" :menu-props="{ class: 'large-select-menu' }"
                  required :rules="[v => !!v || '必填']" @update:model-value="formData.subCategory = null"></v-select>
              </v-col>
              <v-col cols="12" md="4">
                <v-select v-model="formData.subCategory" :items="filteredSubCategories" item-title="value"
                  item-value="value" label="工程細項" variant="outlined" class="large-text-select"
                  :menu-props="{ class: 'large-select-menu' }" required :rules="[v => !!v || '必填']"
                  :disabled="!formData.category" no-data-text="請先選擇工程種類"></v-select>
              </v-col>

              <v-col cols="12">
                <v-label class="mb-1">快選說明 (點擊加入)</v-label>
                <div class="d-flex flex-wrap ga-1">
                  <v-chip v-for="option in options.quickReply" :key="option.id"
                    @click="appendToDescription(option.value)" variant="outlined" color="info">
                    {{ option.value }}
                  </v-chip>
                </div>
              </v-col>
              ...
              <v-col cols="12">
                <v-textarea v-model="formData.description" label="說明狀況" rows="3" variant="outlined"></v-textarea>
              </v-col>

              <v-col cols="12">
                <v-label class="mb-1">檢查狀態</v-label>
                <v-chip-group v-model="formData.status" mandatory column color="primary">
                  <v-chip v-for="option in options.status" :key="option.id" :value="option.value"
                    :color="option.color || 'grey'" filter variant="outlined">
              ...
              <v-col cols="12">
                ...
                <v-chip-group v-model="formData.level" mandatory column color="primary">
                  <v-chip v-for="option in options.level" :key="option.id" :value="option.value"
                    :color="option.color || 'grey'" filter variant="outlined">
                    {{ option.value }}
                  </v-chip>
                </v-chip-group>
              ...
              <v-col cols="12">
                ...
                <v-chip-group v-model="formData.progress" mandatory column color="primary">
                  <v-chip v-for="option in options.progress" :key="option.id" :value="option.value"
                    :color="option.color || 'grey'" filter variant="outlined">
                    <v-icon v-if="option.icon" start size="small">{{ option.icon }}</v-icon>
                    {{ option.value }}
                  </v-chip>
                ...
                <div class="d-flex align-center ga-2">
                  <v-tooltip location="top" :text="formData.customerView ? '驗屋報告將顯示此紀錄' : '驗屋報告不顯示此紀錄'">
                    <template v-slot:activator="{ props }">
                      <v-btn v-bind="props" :icon="formData.customerView ? 'mdi-eye-outline' : 'mdi-eye-off-outline'"
                        :color="formData.customerView ? 'primary' : 'grey'" variant="outlined"
                        @click="formData.customerView = !formData.customerView" aria-label="切換客戶檢視狀態"></v-btn>
                    </template>
                  </v-tooltip>
              ...
              <v-col cols="12" v-if="isEditMode">
                <div class="mt-4">
                  <v-label class="mb-1">客戶確認狀態</v-label>
                  <div class="mb-2"> <v-chip v-if="!formData.customerConfirmedAt" color="red" text-color="white"
                      size="small" label>
                      客戶未確認
                    </v-chip>
                  ...
                  <div>
                    <v-btn v-if="formData.customerConfirmedAt" color="error" variant="outlined" size="small"
                      @click="clearBuyerConfirmation" :disabled="isSaving || isClearingConfirmation"
                      :loading="isClearingConfirmation" prepend-icon="mdi-eraser">
                      清除客戶確認
                    </v-btn>
                  </div>
                </div>
      ...
      <v-card-actions class="pa-3">
        ...
        <v-btn color="grey" variant="text" @click="closeDialog"
          :disabled="isSaving || isClearingConfirmation">取消</v-btn>
        <v-btn color="primary" variant="flat" :loading="isSaving" :disabled="!valid || isClearingConfirmation"
          @click="saveRecord">儲存紀錄</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
async function clearBuyerConfirmation() {
  ...
  if (!confirm('您確定要清除此紀錄的客戶確認狀態嗎？此操作無法復原。')) {
    return;
  }
  ...
  try {
    ...
    console.error("清除客戶確認時發生錯誤:", error);
