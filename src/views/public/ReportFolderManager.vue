...
function downloadPreviewFile() {
  ...
  try {
    ...
    // 必須在使用者點擊的同步呼叫堆疊內觸發開窗，
    // 否則手機 Safari / LINE 內建瀏覽器會靜默封鎖彈窗
    if (liff.isInClient()) {
      try {
        // LIFF 內建瀏覽器無法直接下載檔案，交給外部瀏覽器處理
        liff.openWindow({ url: downloadUrl, external: true });
        return;
      } catch (e) {
        // liff 未初始化時 openWindow 會拋錯，改走 anchor 下載
        console.warn('liff.openWindow 失敗，改用 anchor 下載:', e);
      }
    }
