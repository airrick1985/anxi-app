<template>
  <v-container fluid class="pa-0" style="background-color: #F4F4F7; min-height: 100vh; padding-bottom: 72px;">
      ...
      <div v-else>
        ...
        <div v-else>
          <div v-if="viewMode === 'table'">
            <v-data-table :headers="headers" :items="filteredRecords" item-value="id" class="elevation-0"
              :items-per-page="10"
                  ...
                  <td>
                    <div class="d-flex ga-1 pa-1">
                      ...
                      <div v-if="item.photos.length > 4"
                        class="d-flex align-center justify-center text-caption text-grey" style="width: 40px;">
                ...
                <v-card class="mb-3 record-card" variant="outlined"
                  ...
                  <v-card-item class="pb-1 pt-2">
                    <div>
                      ...
                      <span class="text-caption text-grey">{{ formatDate(item.inspectionDate) }} - {{ item.phase
                        }}</span>
                    </div>
                  ...
                  <v-card-text class="py-2">
                    <div class="d-flex ga-1 flex-wrap mb-1">
                      <ChipRenderer size="x-small" :value="item.status" type="status"
                        :options="optionsForChips.status" />
                    </div>
    ...
    <v-bottom-navigation fixed color="primary" elevation="8" grow mandatory v-model="bottomNavValue">
    ...
    <v-dialog v-model="showConfirmDialog" persistent fullscreen transition="dialog-bottom-transition">
      <v-card>
        ...
        <v-card-text class="pa-4 confirm-scrollable-content">
          ...
          <v-row dense>
            <v-col cols="12" md="4">
              <v-text-field v-model="buyerInfo.name" label="客戶姓名 *" variant="outlined" :rules="[rules.required]"
                :loading="isLoadingBuyerInfo" :readonly="isSaving"></v-text-field>
            </v-col>
            <v-col cols="12" md="4">
              <v-text-field v-model="buyerInfo.phone" label="連絡電話 *" variant="outlined" :rules="[rules.required]"
                :loading="isLoadingBuyerInfo" :readonly="isSaving"></v-text-field>
            </v-col>
            <v-col cols="12" md="4">
              <v-text-field v-model="buyerInfo.email" label="Email *" variant="outlined" hint="請確認 Email 是否正確，以免收不到驗屋報告"
                persistent-hint type="email" :rules="[rules.required, rules.email]" :loading="isLoadingBuyerInfo"
                :readonly="isSaving"></v-text-field>
            </v-col>
          </v-row>
          ...
          <div class="signature-pad-container border rounded" :class="{ 'shake-animation': shakeSignaturePad }"
            @touchstart.stop.prevent @touchmove.stop.prevent>

            <VueSignaturePad ref="signaturePad" width="100%" height="220px"
        ...
        <v-card-actions class="pa-4 confirm-dialog-footer"> <v-spacer></v-spacer>
          <v-btn color="grey-darken-1" variant="text" @click="closeConfirmDialog" :disabled="isSaving">
            關閉
          </v-btn>
          <v-btn @click="saveConfirmation" :disabled="!isConfirmReady || isSaving" :loading="isSaving" color="primary"
...
const headers = computed(() => [
  ...
  { title: '客戶確認', key: 'customerConfirmedAt', sortable: true }, // ✅ 新增欄位
...
async function loadBuyerInfo() {
  ...
  try {
    ...
    if (result.status === 'success' && result.data) {
      ...
      console.log('成功載入預設客戶資訊。');
    } else {
      console.warn("無法獲取預設客戶資訊:", result.message || 'API 回傳非成功狀態');
    }
  } catch (error) {
    console.error("載入預設客戶資訊時發生 API 錯誤:", error);
  } finally {
    isLoadingBuyerInfo.value = false;
  }
}
...
.confirm-fab {
  position: fixed;
  bottom: 28px;
  /* 位於底部導航列 (約 56px) 之上，與 InspectionConsole.vue 中的 FAB 相似 */
  right: 16px;
  /* 靠右對齊 */
  z-index: 100 !important;
  /* 確保在 v-bottom-navigation (z-index 8) 之上 */
}
...
.confirm-dialog-footer {
  ...
  /* 添加陰影分隔 */
  z-index: 2001;
  /* 確保它在 v-dialog (z-index 約 2000) 之上 */
