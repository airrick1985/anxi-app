...
async function save() {
  ...
  try {
    if (isEditing.value) {
      await updateSubscription(editedItem.value.SubscriptionID, editedItem.value, adminKey.value);
    } else {
      await addSubscription(editedItem.value, adminKey.value);
    }
    alert('儲存成功！');
    closeDialog();
    await onMounted(); // 重新載入所有資料
  } catch (error) {
